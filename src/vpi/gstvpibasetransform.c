...
G_DEFINE_ABSTRACT_TYPE(GstVpiBaseTransform, gst_vpi_base_transform, GST_TYPE_BASE_TRANSFORM);
...
static void gst_vpi_base_transform_init(GstVpiBaseTransform *self)
{
  ...
  gst_base_transform_set_in_place (GST_BASE_TRANSFORM (self), FALSE);
    ...
    self->cuda_mem_type = NVBUF_MEM_DEFAULT;
  #else
    self->cuda_mem_type = NVBUF_MEM_CUDA_UNIFIED;
  #endif
}
...
static GstFlowReturn gst_nv_spherical_warper_transform (GstBaseTransform * base, GstBuffer * inbuf, GstBuffer * outbuf)
{
  GstVpiBaseTransform *self = GST_VPI_BASE_TRANSFORM(base);
  NvBufSurface *surface, *surface_out = NULL;
  GstVpiBaseTransformClass *bclass = GST_VPI_BASE_TRANSFORM_GET_CLASS (trans);
    ...
    GstBuffer * inbuf, GstBuffer ** outbuf)
{
  ...
  GstNVSphericalWarper *self = GST_NV_SPHERICAL_WARPER(trans);

  result = gst_buffer_pool_acquire_buffer (self->pool, &gstOutBuf, NULL);
  GST_DEBUG_OBJECT (self, "%s : Frame=%lu Gst-OutBuf=%p\n",
		  __func__, self->frame_num, gstOutBuf);
...
static gboolean
gst_vpi_base_transform_size(GstBaseTransform* btrans,
...
static gboolean gst_vpi_base_transform_stop(GstBaseTransform * trans){
  ...
  gst_nv_spherical_warper_release(self);
  if (self->pool) {
...
static gboolean gst_vpi_base_transform_set_caps (GstBaseTransform * btrans, GstCaps * incaps,
    GstCaps * outcaps)
{
  GstNVSphericalWarper *self = GST_NV_SPHERICAL_WARPER(btrans);
  ...
  self->output_fmt = GST_VIDEO_FORMAT_INFO_FORMAT (self->out_info.finfo);
  ...
  if (!self->pool)
  {
    ...
    gst_structure_set (config,
        "memtype", G_TYPE_UINT, self->cuda_mem_type,
        "gpu-id", G_TYPE_UINT, self->gpu_id,
        "batch-size", G_TYPE_UINT, self->batch_size, NULL);

    GST_INFO_OBJECT (self, " %s Allocating Buffers in NVM Buffer Pool for Max_Views=%d\n",
        __func__, self->batch_size);
...
static void gst_vpi_base_transform_class_init(GstVpiBaseTransformClass *klass)
{
  ...
  base_transform_class->transform = GST_DEBUG_FUNCPTR(gst_nv_spherical_warper_transform);
